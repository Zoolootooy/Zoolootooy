...
function addAdvert($title, $description, $phone, $country_id, $email, $end_date,  $filename){
    ...
    //запрос с метками
    $sql = "INSERT INTO advert (title, description, phone, country_id, email, end_date, create_date, photo) VALUES (:title, 
:description, :phone, :country_id, :email, :end_date, :create_date, :photo)";
...
function getAdvert($id){
    $pdo = new PDO("mysql:host=localhost;dbname=bulletinboard", "MyRoot", "");
    $statement = $pdo->prepare("SELECT * FROM advert WHERE advert.id =".$id);
